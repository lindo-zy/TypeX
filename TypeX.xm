...
static NSString *gTypeXSBLastConsumedRequestID = nil;
static CFAbsoluteTime gTypeXSBLastExecuteTime = 0;

// Apple schemes that LaunchServices may refuse to resolve and that only
// SpringBoard itself may open.
...
static void TypeXSBOpenRequestCallback(CFNotificationCenterRef center,
    ...
    if ([requestID isEqualToString:gTypeXSBLastConsumedRequestID]) {
    ...
    if (gTypeXSBLastExecuteTime > 0 && now - gTypeXSBLastExecuteTime < TypeXSBExecuteThrottle) {
        NSLog(@"[TypeXSB] throttled burst, dropped id=%@", requestID);
        TypeXSBReportStatus(requestID, NO, @"throttled");
        return;
    }

    gTypeXSBLastConsumedRequestID = requestID;
    gTypeXSBLastExecuteTime = now;
