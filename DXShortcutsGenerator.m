...
// ShellX 截图按钮依赖 com.iosdump.shellx 在场且总开关已开启（GlobalEnabled），
// 否则通知发过去会被其 SpringBoard 端守卫直接丢弃，按钮形同虚设。
// CFPreferences 跨域读取该键：键不存在视为未安装/未启用，从动作目录中隐藏。
+(BOOL)isShellXScreenshotAvailable{
    static BOOL available;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        available = (CFPreferencesCopyAppValue(CFSTR("GlobalEnabled"), CFSTR("com.iosdump.screenshotshell")) != nil);
    });
    return available;
}

// AI 对话按钮只要求 ShellX 插件在场（不要求其总开关 GlobalEnabled：悬浮 AI
// 面板不经过该守卫）。用 dylib 落盘路径探测，比读它的偏好域更可靠——用户从未
// 打开过 ShellX 设置时其域可能还没有任何键。
+(BOOL)isShellXAIChatAvailable{
    static BOOL available;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        available = [[NSFileManager defaultManager] fileExistsAtPath:DX_ROOT_PATH_NS(@"/Library/MobileSubstrate/DynamicLibraries/ScreenshotShell.dylib")];
    });
    return available;
}
