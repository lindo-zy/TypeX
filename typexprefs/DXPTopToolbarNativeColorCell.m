...
@implementation DXPTopToolbarNativeColorCell
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated {
    ...
    if (selected) {
        ...
        if ([ctrl isKindOfClass:[DXPTopToolbarSettingsController class]]) {
            DXPTopToolbarSettingsController *settingsCtrl = (DXPTopToolbarSettingsController *)ctrl;
            NSString *key = [self.specifier propertyForKey:@"key"];
            [settingsCtrl presentSystemColorPickerForKey:key ?: @"toptoolbarbackgroundtint"];
        }
    }
}
